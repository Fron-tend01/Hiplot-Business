...
const Header: React.FC = () => {
  ...
  const setNotfys = storeHeader(state => state.setNotfys)
  const { toggle, notfys } = storeHeader()
  ...
  const setPermisosxVistaheader = storeDv((state) => state.setPermisosxVistaheader);
  ...
  const getNotificaciones = async () => {
    await APIs.GetAny('get_notificaciones/' + user_id).then((r: any) => {
      setNotfys(r)
    })
  };
  // useEffect(() => {
      ...
      <div className='container__hero'>
        ...
        <div className='nav__hero'>
          ...
          <div className='container__bell'>
            <small className='number__bell'>{notfys.length}</small>
