...
const ModalCreate: React.FC = () => {
    ...
    const {modalStateCreate, setModalStateCreate, getStore}: any = storeStore()
    ...
    const [sucursales_eliminar] = useState<any>([])
    ...
    const handleCompaniesChange = async (company: any) => {
      ...
      const modalCreateCompanies = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        let filter = sucursales_nuevas.map((x: any) => x.id);
        let data_ext = {
            sucursales_nuevas: filter,
            sucursales_eliminar
        };
        try {
          await createStore(data, data_ext);
      ...
      const addBranchOfice = () => {
        ...
        setSucursales_nuevas([...sucursales_nuevas, selectedBranchOffice]);
      }
  ...
  return (
    <div className={`overlay__store ${modalStateCreate ? 'active' : ''}`}>
        <Toaster expand={true} position="top-right" richColors  />
        <div className={`popup__store ${modalStateCreate ? 'active' : ''}`}>
            ...
            <div className='conatiner__table_store'>
                <div className='table__modal_store'>
                    ...
                    <div className='container__branchOffice_table-modal'>
                        {sucursales_nuevas.map((sucursal: any) => (
                ...
                <div className='container__btn_create-store'>
                    <button className='btn__general-purple' type='submit' >Crear Almacen</button>
                </div>
            </div>
            </form>
        </div>
