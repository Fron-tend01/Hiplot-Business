...
const Users: React.FC = () => {
  ...
  const {createUsers, users, getUsers, putUsers }: any = storeUsers();
...
const toggleCheckbox = (sucursal_id: any, area_id: any) => {
  ...
  const addUpdateUsers = () => {
    if (selectUser !== null) {
      const user = users.find((x: any) => x.id === selectUser);
      if (user) {
  ...
  const handleUpdateUsers = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateBranchPermissions.forEach((x: any) => {
      setSucursalesNuevas(prevGrupos => [...prevGrupos, x.id]);
    });

    updateAreasPermissions.forEach((x: any) => {
    ...
    usersDelete.forEach((x: any) => {
      ...
      await putUsers(user, sucursal_id, nombre, email, password, tipo_us, sucursales_nuevas, sucursales_eliminar, areas_nuevas, areas_eliminar, subordinados_nuevos, subordinados_eliminar, grupos_nuevos, grupos_eliminar);
    ...
    <div className='users'>
        ...
        <div className={`popup__users ${modal ? 'active' : ''}`}>
          ...
          <form className='conatiner__create_users' onSubmit={handleCreateUsers}>
            ...
            <div className='row__three'>
              <div className='users__form_row-three-left'>
                ...
                <div className='users__form_row-three-right'>
                  ...
                  <div className='select__container'>
                    ...
                    <div className='select-btn__general'>
                      <div className={`select-btn ${selectUsers ? 'active' : ''}`} onClick={openSelectUsers}>
                        <p>{selectUser ? users.find((s: {id: number}) => s.id === selectUser)?.nombre : 'Selecciona'}</p>
                        <svg className='chevron__down' xmlns="http://www.w3.org/2000/svg"  height="16" width="16" viewBox="0 0 512 512"><path d="M233.4 406.6c12.5 12.5 32.8 12.5 45.3 0l192-192c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L256 338.7 86.6 169.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l192 192z"/></svg>
                      </div>
                      <div className={`content ${selectUsers ? 'active' : ''}`}>
                        <ul className={`options ${selectUsers ? 'active' : ''}`} style={{ opacity: selectUsers ? '1' : '0' }}>
                          ...
                          {users && users.filter((user: any) => user.nombre.toLowerCase().includes(userSearchTerm.toLowerCase()))
                            .map((user: any) => (
                          ...
                          <div className='tbody__container_users_right'>
                            {usersExist.map((user: any) => (
                              <div className='tbody__users_right' key={uuidv4()}>
                                <div className='td'>
                                  <li className='text'>
                                    {users.find((user_id: any) => user_id.id === user)?.nombre}
                                  </li>
                                </div>  
                                <div className='td'>
                                  <li className='text'>
                                    {users.find((user_id: any) => user_id.id === user)?.email}
                                  </li>
                                </div>
                                <div>
      ...
      <div className={`overlay__edit_users ${modalUpdate ? 'active' : ''}`}>
          <div className={`popup__edit_users ${modalUpdate ? 'active' : ''}`}>
            ...
            <h3 className='title__modals'>Crear tipo de usuario</h3>
                            ...
                            <div className='tbody__container_users_right'>
                              {usersExist.map((user: any) => (
                                <div className='tbody__users_right' key={uuidv4()}>
                                  <div className='td' >
                                    <li>
                                      {users.find((user_id: any) => user_id.id === user.id)?.nombre}
                                    </li>
                                  </div>
                                  <div className='td'>
                                    <li>
                                      {users.find((user_id: any) => user_id.id === user.id)?.email}
                                    </li>
                                  </div>
                                  <div>
