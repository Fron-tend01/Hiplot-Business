...
const UpdateModal = () => {
    ...
    useEffect(() => {
        if (typeOfPaymentToUpdate) {
            ...
            console.log(typeOfPaymentToUpdate.operaciones_obj);
    ...
    useEffect(() => {
        if (typeOfPaymentToUpdate) {
            let campop: any[] = []
            typeOfPaymentToUpdate.operaciones_obj.forEach((obj: any) => {
                const maxId = typeOfPaymentToUpdate.operaciones_obj.reduce((max: any, operation: any) => Math.max(max, operation.id), 0);
                const nextId = maxId + 1;
                const nextIdForCampo = nextId + 1;
                let campo = obj.tipo == 0 ? {
                    title: 'Campo',
                    id: nextId,
                    defaultId: obj.id_plantillas_art_campos,
                    fields: templatesFields,
                    selectTypes: false,

                    id_plantillas_art_campos: obj.id_plantillas_art_campos,
                    tipo: obj.tipo,
                    ...
                    id: nextIdForCampo,
                    name: '+',
                    defaultId: typeOfPaymentToUpdate ? obj.id_plantillas_art_campos : null,
                    selectTypes: false,
                    fields: [
                        {
                            id: 0, nombre: '+',
    ...
    return (
        <div className={`overlay__create_modal_type-payment ${modal == 'update_type-payment' ? 'active' : ''}`}>
            <div className={`popup__create_modal_type-payment ${modal == 'update_type-payment' ? 'active' : ''}`}>
                ...
                <p className='title__modals'>Actualizar tipo de cobro</p>
                <form className='conatiner__create_modal_type-payment' onSubmit={handleCreateTypeOfPayments}>
                    <div className='row'>
                        ...
                        <div className='col-12'>
                                <button className='btn__general-purple' type='button' onClick={addFields} >Añadir más campos al tipo de cobro</button>
                        </div>
                    </div>

                    <div className='row'>
                        {operations?.map((x: any, index: any) => (
                            <div key={`${x.id}-${index}`} className='col-3 md-col-6 sm-col-12'>
