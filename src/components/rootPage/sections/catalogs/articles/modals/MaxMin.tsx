...
const MaxMin: React.FC = () => {
  ...
  const addMaxMin = async () => {
    ...
    let filter = await maxsMins.find((x: any) => x.id_sucursal == selectedMaxMinBranchOffice)

    if(filter) {
        setWarinings('maxsmins')
        toast.warning('La sucursal ya tiene un almacén agregado')
        } else {
        setWarinings('')
        let dataMaxMin = {
            id_sucursal: selectedMaxMinBranchOffice,
        ...
        <div className='article__modal_save_modal_maxmin_container'>
            <div className='row__one'>
                <div className='select__container'>
                    ...
                    <div className={`select-btn ${selectMaxMinCompanies ? 'active' : ''}`} onClick={openSelectMaxMinCompanies}>
                        <div className='select__container_title'>
                            <p>{selectedMaxMinCompany ? companiesXUsers.find((s: {id: number}) => s.id === selectedMaxMinCompany)?.razon_social : 'Selecciona'}</p>
                        </div>
                        <svg className='chevron__down' xmlns="http://www.w3.org/2000/svg"  height="16" width="16" viewBox="0 0 512 512"><path d="M233.4 406.6c12.5 12.5 32.8 12.5 45.3 0l192-192c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L256 338.7 86.6 169.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l192 192z"/></svg>
                    </div>
                    <div className={`content ${selectMaxMinCompanies ? 'active' : ''}`}>
                        <ul className={`options ${selectMaxMinCompanies ? 'active' : ''}`} style={{ opacity: selectMaxMinCompanies ? '1' : '0' }}>
                        {companiesXUsers && companiesXUsers.map((company: any) => (
                            <li key={company.id} onClick={() => handleMaxMinCompaniesChange(company.id)}>
