...
const Differential: React.FC = () => {
  ...
  const addArticle = async () => {
    const data = {
      id_proveedor: selectedSupplier || 0,
    ...
    const resultArticle = await getArticlesDifferential(data).finally(() => {
      setModalLoading(false)
    ...
    let for_insert = []
    resultArticle.forEach(element => {
      let max;
      if (element.max_mins.find((x: any) => x.id_sucursal == selectedBranchOffice)) {
        console.log('Si tiene la  sucursal')
        const mm_tmp = element.max_mins.filter((x: any) => x.id_sucursal == x.id_sucursal)
        element.max_min = mm_tmp[0].maximo + ' - ' + mm_tmp[0].minimo
        max = mm_tmp[0].maximo + ' - ' + mm_tmp[0].minimo
      } else {
        max = element.max_min = 'N/A'
      }

      const unidad = element.unidades.filter((x: any) => x.unidad_almacen === true);


      const dataAticle = {
        id_articulo: element.id,
      ...
      for_insert.push(dataAticle)
    });
    setConcepts(for_insert);
